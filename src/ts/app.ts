...
function countBill() {
	...
	const numberOfPeople: number = parseFloat(peopleInput.value);
	const customInputPercentage: number = parseFloat(customInput.value);
	let btnActive: number = parseFloat((document.querySelector('button.show') as HTMLButtonElement).value);
	
	if (customInputPercentage) {
		btnActive = parseFloat((document.querySelector('#custom') as HTMLInputElement).value);
	}

	const totalPrice: number = (btnActive / 100) * parseFloat(billValue.toFixed(2));
